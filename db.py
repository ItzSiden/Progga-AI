...
def get_all_nodes():
    return fetchall(
        "SELECT id, node_type, node_name, parent_id, path, created_at FROM hierarchy_nodes ORDER BY path ASC"
    )


def get_custom_tables(path: str | None = None):
    if path:
        return fetchall(
            "SELECT id, table_name, path, columns_json, created_at FROM data_tables WHERE path=? ORDER BY table_name",
            (path,),
        )
    return fetchall(
        "SELECT id, table_name, path, columns_json, created_at FROM data_tables ORDER BY path, table_name"
    )


def get_custom_rows(table_id: int):
    return fetchall(
        "SELECT id, row_json, created_at FROM data_rows WHERE table_id=? ORDER BY id DESC",
        (table_id,),
    )
